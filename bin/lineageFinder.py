...
class LineageFinder:
    ...
    def get_lineage(self):
        ...
        if lineage is None:
            if self.query_node_parent is not self.tree.seed_node:
                ...
                # the parent node is the root and the ancestral lineage is A
                lineage = "A"
        if lineage == "A":
            self.at_root = True
    ...
    def get_label(self):
        ...
        if self.at_root:
            ...
            for node in self.tree.preorder_node_iter(lambda n: n.annotations.get_value("lineage") is not None and (n.annotations.get_value("lineage") == "B" or (len(n.annotations.get_value("lineage").split(".")) == 2 and n.annotations.get_value("lineage")[0] == "A"))):
            ...
            lineage_mrca = [node for node in self.tree.preorder_node_iter(lambda n: n.annotations.get_value("lineage") == self.lineage)][0]

            label = lineage_mrca.label # if lineage_mrca.label is not None else lineage_mrca.annotations.get_value("label")
            if label is None:
                alrt, boot = 0.0, 0.0
            else:
                label = lineage_mrca.label  
                alrt, boot = label.split("/")[:2]
                # print(label, alrt, boot)
                
            self.alrt = int(float(alrt))
            self.boot = int(float(boot))

                
            # if lineage_mrca.label is not None:
            #     label = lineage_mrca.label
            #     # alrt, boot = label.split("/")[-2:]
            #     # self.alrt = int(float(alrt))
            #     # self.boot = int(float(boot))
            # else:
            #     label = lineage_mrca.annotations.get_value("label")
...
def parse_tree(file, format):
    return Tree.get(path=file, schema=format.lower(), preserve_underscores=True)
